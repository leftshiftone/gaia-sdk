...
class HttpSensorFunction(url: String, credentials: GaiaCredentials, transporterFactory: TransporterFactory) : ISensorFunction {
    ...
    override fun retrieveIdentityMetrics(identityId: Uuid, startDate: String, config: IdentityMetrics.() -> Unit, limit: Int?): Publisher<gaia.sdk.response.type.IdentityMetrics> =
            map(client.query(GaiaRequest.query { retrieve { experience { identityMetrics(identityId, startDate, limit, config) } } })) { it.retrieve?.experience?.identityMetrics!! }
